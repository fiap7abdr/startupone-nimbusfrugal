...
| 🟡 R2.15 | Somente owner do tenant pode renomear o tenant (UI inline em `/app/settings` + guard server-side `ownerUserId === user.id`, 2–80 chars, audit `tenant.renamed`) | Must-have |
...
| 🟡 R2.15 | Somente owner do tenant pode renomear o tenant | Must-have | ✅ |
| R3.1 | Gerenciar múltiplas AWS Organizations (adicionar/editar/remover) em página única `/app/integrations` | Must-have | ✅ |
| R3.2 | CloudFormation gerado por integração (por conector, não por org inteira) | Must-have | ✅ |
| R3.3 | Trust policy referencia AWS Account ID da plataforma Nimbus | Must-have | ✅ |
...
- 🟡 Rename do tenant em `/app/settings` (owner-only, inline): server action `renameTenant` + audit `tenant.renamed`
